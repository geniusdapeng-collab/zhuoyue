...
1. 在原始Prompt末尾追加以下内容（不要删除原内容）：
   ...
   - 山海经世界特征：符合神话设定的环境、异兽纹理、神奇植被
   - 环境音效暗示：风声、生物共鸣、环境氛围
2. 追加后总长度应达到2500-3000字符（卓越系统）
3. 保持中文纪录片质感，直接追加到原Prompt后面

直接输出追加后的完整Prompt，不要解释，不要JSON。`;
  } else {
    // v6.6.9.4-patch14: Generic模式prompt去Nirath化，强制中文标准字段输出
    return `你是顶级AI视频Prompt工程师，擅长在现有Prompt基础上补充细节。
...
【补充要求】
1. 在原始Prompt末尾追加以下内容（不要删除原内容）：
   - 环境质感：真实环境细节、材质纹理、空间感
   - 光影变化：自然光照、色温、阴影层次
   - 角色微表情：眼神、呼吸、面部细节
   - 真实世界特征：符合物理规律、真实材质、自然纹理
   - 环境音效暗示：环境声、空间混响
2. 追加后总长度应达到2500-3000字符（卓越系统）
3. 保持中文纪录片/科普质感，直接追加到原Prompt后面
4. **输出格式要求**：必须使用中文标准字段，格式如下：
   【视觉】... | 【动态】... | 【空间】... | 【情绪】... | 【纵深】... | 【方位】... | 【风格】... | 【镜头时间轴】... | 【照明】... | 【负面约束】... | 【环境音效】... | 【渲染】... | 【导演】... | 【台词】"台词内容" | 【人物介绍卡片】...
