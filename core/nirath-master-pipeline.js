...
- 场景名称是中文,台词内容也必须匹配中文场景名所暗示的主题`;;
  ...
  // ========== Stage 6: 时长分配(集成ShotDurationAllocatorV3 + DurationCalculator双保险 + P1修复) ==========
  async stageDurationAllocation(script, input) {
    ...
    try {
      if (typeof this.modules.shotDurationAllocator.allocate === 'function') {
        ...
        if (isCommercial && rhythmCurve === 'classic') {
          ...
          this.log('STAGE-6', `🎬 商业广告模式: 使用肾上腺素式节奏曲线`);
        }
        ...
        if (this.modules.durationConstraint && isCommercial) {
          ...
          this.log('STAGE-6', `⏱️ 时长约束系统: 总时长${finalDuration}秒`);
        }
    ...
    if (this.modules.durationConstraint && input._enrichedSellingPoints) {
      ...
      if (!validation.valid) {
        this.log('STAGE-6', `⚠️ 时长约束验证失败: ${validation.issues.join(', ')}`);
      } else {
        this.log('STAGE-6', `✅ 时长约束验证通过: ${validation.totalDuration}秒`);
      }
    }
    ...
    if (input._enrichedSellingPoints) {
      ...
      this.log('STAGE-6', `📊 广告阶段时长分布: ${Object.entries(phaseBreakdown).map(([k,v]) => `${k}:${v}s`).join(' | ')}`);
    }
